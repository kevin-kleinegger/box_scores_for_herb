...
- [x] 11. Implement box score generator
  - [x] 11.1 Create BoxScoreGenerator class
    - Write services/box_score_generator.py with generate_for_date(), _prioritize_mets(), _find_most_recent_game_date() methods
    - Implement Mets prioritization logic (Mets game first if present)
    - Handle offseason dates: if no games found for requested date, find and display most recent game date using coarse-to-fine search
    - Fetch formatted box score text using statsapi.boxscore()
    - Generate linescore (inning-by-inning scoring) with _create_linescore_string()
    - _Requirements: 17.1, 17.2, 17.3, 15.3_
  
  - [x] 11.2 Write integration tests for BoxScoreGenerator
    - Test box score generation for regular season date
    - Test box score generation for spring training date
    - Test box score generation for offseason date with fallback
    - All tests passing
    - _Requirements: 10.5_
  
  - [ ]* 11.3 Write property test for Mets prioritization
    - **Property 14: Mets Game Prioritization**
    - **Validates: Requirements 15.3**
  
  - [ ]* 11.4 Write property test for non-Mets game ordering
    - **Property 18: Non-Mets Game Ordering Consistency**
    - **Validates: Requirements 17.3**

- [x] 12. Implement standings generator
  - [x] 12.1 Create StandingsGenerator class
    - Write services/standings_generator.py with generate_for_date() method
    - Fetch AL and NL standings separately using statsapi.standings() for side-by-side display
    - Use cached API client methods (get_standings_text_by_league)
    - _Requirements: 17.4, 19.1, 19.3_
  
  - [x] 12.2 Write integration tests for StandingsGenerator
    - Test standings generation for regular season date
    - Test standings generation for spring training date
    - Test standings generation for offseason date
    - All tests passing
    - _Requirements: 10.5_
  
  - [ ]* 12.3 Write property test for standings data completeness
    - **Property 19: Standings Data Completeness**
    - **Validates: Requirements 19.2**

- [x] 13. Implement leaderboard generator with optimization
  - [x] 13.1 Create LeaderboardGenerator class
    - Write services/leaderboard_generator.py with generate_all(), _detect_current_season(), _fetch_standard_stat_leaders(), _fetch_custom_stat_leaders() methods
    - For standard stats (AVG, HR, RBI, OBP, SLG, OPS, 2B, 3B, SB, R, ERA, W, K, SV, IP, WHIP): use `/stats/leaders` endpoint (1 API call per stat)
    - For custom stats (TBR, TBR+): fetch OPS leaders as base (top 100 players), then calculate TBR/TBR+ using StatsCalculator
    - Cache all player stats for 24 hour reuse
    - Total API calls: ~113 (12 standard stats + 1 OPS + 100 individual player stats)
    - Performance: 19s first run, 0.26s cached (73x speedup)
    - _Requirements: 4.1, 4.2, 4.3, 12.1, 12.2, 12.3, 12.4_
  
  - [x] 13.2 Write integration tests for LeaderboardGenerator
    - Test leaderboard generation for current season
    - Test leaderboard generation for previous season
    - Test season detection logic
    - All tests passing
    - _Requirements: 10.5_
  
  - [ ]* 13.3 Write property test for stat definition metadata
    - **Property 11: Stat Definition Metadata**
    - **Validates: Requirements 11.5**
  
  - [ ]* 13.4 Write property test for leaderboard stat flexibility
    - **Property 12: Leaderboard Stat Flexibility**
    - **Validates: Requirements 12.1, 12.3**
  
  - [ ]* 13.5 Write property test for qualification threshold
    - **Property 13: Qualification Threshold Application**
    - **Validates: Requirements 12.4**

- [ ] 14. Create cache warming script
  - [x] 14.1 Create warm_cache.py script
    - Script pre-fetches all data (box scores, standings, leaderboards) for instant page loads
    - Takes ~16 seconds to warm cache, then all requests are <1 second
    - Designed to run once daily via PythonAnywhere scheduled tasks
    - All cache TTL settings updated to 24 hours for newspaper-style daily updates
